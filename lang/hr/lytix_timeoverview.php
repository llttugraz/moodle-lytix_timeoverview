<?php
// This file is part of Moodle - http://moodle.org/
//
// Moodle is free software: you can redistribute it and/or modify
// it under the terms of the GNU General Public License as published by
// the Free Software Foundation, either version 3 of the License, or
// (at your option) any later version.
//
// Moodle is distributed in the hope that it will be useful,
// but WITHOUT ANY WARRANTY; without even the implied warranty of
// MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
// GNU General Public License for more details.
//
// You should have received a copy of the GNU General Public License
// along with Moodle.  If not, see <http://www.gnu.org/licenses/>.

/**
 * Strings for component 'lytix_timeoverview', language 'hr'.
 *
 * @package   lytix_timeoverview
 * @copyright 2023 Educational Technologies, Graz, University of Technology
 * @license   http:www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
$string['pluginname'] = "Lytix Timeoverview";
$string['privacy:metadata'] = "Ovaj plug-in dodatak ne pohranjuje nikakve podatke.";
$string['cachedef_timeoverview'] = "Privremena pohrana za subplug-in dodatak timeoverview.";
$string['cron_refresh_lytix_timeoverview_cache'] = "Osvježi privremenu pohranu za lytix subplug-in dodatak timeoverview basic";
$string['overview_activities'] = "Pregled vremena";
$string['timeoverview_description'] = "Prikazuje srednji postotak vremena koje studenti provode na raznim aktivnostima.";
$string['timeoverview_label_forum'] = "Forum";
$string['timeoverview_label_quiz'] = "Kviz";
$string['timeoverview_label_video'] = "Video";
$string['timeoverview_label_core'] = "Tečaj";
$string['timeoverview_label_bbb'] = "BigBlueButton";
$string['timeoverview_label_grade'] = "Ocjena";
$string['timeoverview_label_submission'] = "Zadaci";
$string['timeoverview_label_resource'] = "Resursi";
$string['timeoverview_label_feedback'] = "Završni upitnik";
$string['all'] = "Sve";
$string['average_all'] = "prosječno vrijeme";
$string['user_all'] = "moje vrijeme";
$string['all_core'] = "prosjek na tečaju";
$string['user_core'] = "ja na tečaju";
$string['all_forum'] = "prosjek na forumu";
$string['user_forum'] = "ja na forumu";
$string['all_grade'] = "prosjek na ocjenjivanju";
$string['user_grade'] = "ja na ocjenjivanju";
$string['all_submission'] = "prosjek za predaju zadatka";
$string['user_submission'] = "ja za predaju zadatka";
$string['all_resource'] = "prosjek za resurse";
$string['user_resource'] = "ja za resurse";
$string['all_quiz'] = "prosjek za kvizove";
$string['user_quiz'] = "ja za kvizove";
$string['all_bbb'] = "prosjek na BBB-u";
$string['user_bbb'] = "ja na BBB-u";
$string['no_activites_found'] = "Nisu pronađene nikakve aktivnosti za ovaj tečaj.";
$string['sum_user'] = "Zbroj za korisnika: ";
$string['sum_average'] = "Prosječni zbroj: ";
$string['core'] = "Tečaj";
$string['forum'] = "Forum";
$string['grade'] = "Ocjena";
$string['submission'] = "Predani zadatak";
$string['resource'] = "Resurs";
$string['quiz'] = "Kviz";
$string['video'] = "Video";
$string['bbb'] = "BigBlueButton";
$string['label'] = "Oznaka";
$string['feedback'] = "Završni upitnik";
$string['time'] = "Vrijeme";
$string['clicks'] = "Klikovi";
$string['privacy:nullproviderreason'] = "Ovaj plug-in dodatak nema bazu podataka za pohranu korisničkih podataka. On upotrebljava samo API sučelja u mod_assign za pomoć u prikazivanju sučelja ocjenjivanja.";
